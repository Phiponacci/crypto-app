from algorithms.affine import Affine
from tkinter import *
...
class AffineUI:
    def __init__(self, root, menu_site, toolbar_site):
        ...
        self.algorithm = Affine()
        self.create_menu()

    def encrypt(self):
        plain_txt = self.txt_entry.get("1.0", END)
        key_a = self.key_a_entry.get()
        key_b = self.key_b_entry.get()
        try:
            key_a = int(key_a)
            key_b = int(key_b)
        except:
    ...
    def init_key_ui(self):
        # key a
        self.key_a_label = Label(self.root, text="a:")
        self.key_a_label.grid(row=1, column=0, padx=10, pady=10)

        self.key_a_entry = Entry(self.root)
        self.key_a_entry.grid(row=1, column=1, sticky=W, padx=10, pady=10)

        # key b
        self.key_b_label = Label(self.root, text="b:")
        self.key_b_label.grid(row=1, column=2, padx=10, pady=10)

        self.key_b_entry = Entry(self.root)
        self.key_b_entry.grid(row=1, column=3, sticky=W, padx=10, pady=10)
